...
export default function App() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // 修改参数 3: 强制更新模型名称 (ckpt_name)
      // 遍历查找 CheckpointLoaderSimple 节点，确保名称正确
      let ckptUpdated = false;
      const TARGET_CKPT = "Z-Image Turbo fyt";

      for (const [id, node] of Object.entries(workflow)) {
        if ((node as any).class_type === 'CheckpointLoaderSimple') {
           if ((node as any).inputs) {
             (node as any).inputs.ckpt_name = TARGET_CKPT;
             addLog(`已修正模型名称 (Node ${id}): ${TARGET_CKPT}`);
             ckptUpdated = true;
           }
        }
      }
      if (!ckptUpdated) {
        addLog(`[警告] 未找到 CheckpointLoaderSimple 节点，可能无法正确加载模型。`);
      }

      // 注意：已移除删除节点的兼容性清理逻辑，完全保留原始结构
